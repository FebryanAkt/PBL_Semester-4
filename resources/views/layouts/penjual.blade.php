...
<body class="flex min-h-screen flex-col bg-bekas-bg font-sans antialiased text-gray-800">
    ...
    <!-- Script Notifikasi (sama seperti di atas) -->
    @auth
    @if(auth()->user()->isSeller())
    <script>
        async function fetchNotifications() {
            try {
                ...
                data.forEach(notif => {
                    ...
                    html += `
                        <div class="p-3 hover:bg-gray-50 transition cursor-pointer ${bgClass}" data-id="${notif.id}" onclick="markAsRead('${notif.id}', '${d.url}')">
                            <div class="flex gap-3">
                                ...
                                <div class="flex-1">
                                    <p class="text-sm font-semibold text-gray-800">${d.item_name}</p>
                                    <p class="text-xs text-gray-500 mt-1">${d.message.substring(0, 80)}${d.message.length > 80 ? '...' : ''}</p>
