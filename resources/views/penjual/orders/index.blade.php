                    ...
                    <div class="p-5 md:p-6 flex-grow flex gap-4 md:gap-6 items-start border-b md:border-b-0 md:border-r border-gray-100">
                        <div class="w-24 h-24 bg-gray-50 rounded-xl overflow-hidden border border-gray-100 shrink-0">
                            @if($trx->item && $trx->item->image)
                                <img src="{{ asset('images/' . $trx->item->image) }}" alt="{{ $trx->item->name }}" class="w-full h-full object-cover">
                            @else
                        ...
                        <div class="flex-1">
                            ...
                            <h3 class="text-lg font-bold text-gray-800 leading-tight">{{ $trx->item->name ?? 'Barang dihapus' }}</h3>
                            <p class="text-sm text-gray-500 mt-1">Pembeli: <span class="font-medium">{{ $trx->user->name }}</span></p>
                            <p class="text-sm text-gray-500">Total: <span class="font-bold text-bekas-green">Rp {{ number_format($trx->price, 0, ',', '.') }}</span></p>
                        </div>
                    </div>
                    {{-- Bagian kanan: status & aksi --}}
                    <div class="p-5 md:p-6 w-full md:w-64 flex flex-col justify-center bg-gray-50/50 shrink-0">
                        <div class="mb-3">
                            @if($trx->delivery_status == 'belum_dikirim')
                            ...
                            @elseif($trx->delivery_status == 'dikirim')
